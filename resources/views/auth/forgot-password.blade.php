...
<div class="p-4 sm:p-8 sm:bg-white sm:border sm:border-gray-100 sm:rounded-2xl sm:shadow-lg flex flex-col space-y-8 w-full sm:max-w-md">
    ...
    <div class="flex items-center space-x-4">
        ...
        <h1 class="text-xl font-semibold">{{ __('Zresetuj hasło')}}</h1>
    </div>
    {{-- Main --}}
    <div class="flex flex-col space-y-2">
        ...
        <form method="POST" action="{{ route('password.email') }}" class="flex flex-col space-y-4">
            ...
            <div class="pt-2">
                <button type="submit" class="text-sm font-medium px-4 py-2 rounded border border-blue-600 bg-blue-600 text-white hover:bg-blue-500 transition">{{ __('Email Password Reset Link') }}</button>
            </div>
        </form>
    </div>
    {{-- Footer --}}
